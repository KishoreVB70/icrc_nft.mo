...
shared(_init_msg) actor class Example(_args : {
  ...
  private func key(x : Nat32) : Trie.Key<LibraryId> {
    return { hash = x; key = x };
  };

  stable var libraries : Trie.Trie<LibraryId, Library> = Trie.empty();
  stable var users : Trie.Trie<UserId, Library> = Trie.empty();
  ...
  let result: Bool = Map.has(map, nhash, 1);
  let result1: ?Nat = Map.get(map, nhash, 1);
